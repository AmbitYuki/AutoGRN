...
class GTSSupervisor:
    def __init__(self, adj, temperature, **kwargs):
        self._kwargs = kwargs
        ...
        elif self._data_kwargs['dataset_dir'] == 'data/PJM':
            df = pd.read_csv('./data/PJM.csv')
            df = df.set_index(['Datetime'])
            df.index = pd.to_datetime(df.index)
        elif self._data_kwargs['dataset_dir'] == 'data/PJM-aep5':
        ...
        self.adj_knn = torch.Tensor(g).to(device)
        self.adj_dis = torch.Tensor(adj).to(device)

        # # knn adj
        # self.adj_mx = torch.Tensor(g).to(device)
        #
        # # dis adj
        # # self.adj_mx = torch.Tensor(adj).to(device)
        ...
        self.Copula_Model = Copula_Model.cuda() if torch.cuda.is_available() else Copula_Model
    ...
    def _get_log_dir(kwargs):
        ...
        if log_dir is None:
            ...
            if filter_type == 'random_walk':
                ...
                filter_type_abbr = 'DR'
            run_id = 'GTS_%s_%d_h_%d_%s_lr_%g_knn_%d_%s/' % (
                filter_type_abbr, max_diffusion_step, horizon,
                structure, learning_rate, knn_k,
    ...
    def evaluate(self,label, dataset='val', batches_seen=0, gumbel_soft=True):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (x, y) in enumerate(val_iterator):
                ...
                output, mid_output = self.GTS_model(x, self._train_feas, self.adj_dis, self.adj_knn, temp)
                # copulaloss = self.Copula_Model.nll(x=x, y=y, adj=self.adj_knn, logits=output)
                # preds = self.Copula_Model.predict(x = x, y = y, adj = self.adj_knn, logits = output, num_samples=1000)
                # print("preds",preds)
                if label == 'without_regularization': 
                    ...
                    true_label = self.adj_knn.view(mid_output.shape[0] * mid_output.shape[1]).to(device)
            ...
            if dataset == 'test':
                ...
                # print("y_pred", y_pred)
                # print("y_true", y_true)
                # print("y_pred_3", y_pred[2:3])
                # print("y_true_3", y_true[2:3])
                # print("y_pred_6", y_pred[5:6])
                # print("y_true_6", y_true[5:6])
                # print("y_pred_12", y_pred[11:12])
                # print("y_true_12", y_true[11:12])
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for batch_idx, (x, y) in enumerate(train_iterator):
                optimizer.zero_grad()
                x, y = self._prepare_data(x, y)
                output, mid_output = self.GTS_model(x, self._train_feas, self.adj_dis, self.adj_knn, temp, y, batches_seen)
                copulaloss = self.Copula_Model.nll(x = x, y = y, adj = self.adj_dis, logits = output)
                # print("copulaloss",copulaloss)
                # print("2", copulaloss.shape)
                ...
                if label == 'without_regularization':  # or label == 'predictor':
                    ...
                    loss = loss_1 + loss_g + copulaloss
                    losses.append((loss_1.item() + loss_g.item() + copulaloss.item()))
