...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        scores = torch.matmul(q, k.transpose(-2, -1)) / torch.sqrt(torch.tensor(self.hidden_size).float().to(x.device))
...
class EncoderLayer(nn.Module):
    ...
    """
    def __init__(self, d_model, d_ff=None, moving_avg=25, dropout=0.1, activation="relu"):
        super(EncoderLayer, self).__init__()
        d_ff = d_ff or 4 * d_model
        self.attention = SelfAttention(input_size=10,hidden_size=10)
