...
def historical_average_predict(df, period=24 * 7, test_ratio=0.2, null_val=0.):
    """
...
def eval_static(traffic_reading_df):
    logger.info('Static')
    horizons = [1, 3, 5, 7]
    logger.info('\t'.join(['Model', 'Horizon', 'MAE', 'MAPE', 'RMSE']))
    for horizon in horizons:
        ...
        line = 'Static\t%d\t%.2f\t%.2f\t%.2f' % (horizon, mae, mape * 100, rmse)
        logger.info(line)
...
def eval_historical_average(traffic_reading_df, period):
    ...
    for horizon in [1, 3, 6, 12]:
        line = 'HA\t%d\t%.2f\t%.2f\t%.2f' % (horizon, mae, mape * 100, rmse)
        logger.info(line)
...
def eval_var(traffic_reading_df, n_lags=3):
    ...
    logger.info('Model\tHorizon\tMAE\tMAPE\tRMSE')
    for i, horizon in enumerate(n_forwards):
        ...
        line = 'VAR\t%d\t%.2f\t%.2f\t%.2f' % (horizon, mae, mape * 100, rmse)
        logger.info(line)


def main(args):
    traffic_reading_df = pd.read_csv(args.traffic_reading_filename)
    traffic_reading_df = traffic_reading_df.set_index(['Datetime'])
    traffic_reading_df.index = pd.to_datetime(traffic_reading_df.index)
    eval_static(traffic_reading_df)
    eval_historical_average(traffic_reading_df, period=24 * 7)
    eval_var(traffic_reading_df, n_lags=3)


if __name__ == '__main__':
    logger = utils.get_logger('D:\Desktop\qjh\GTS-change\data\model', 'Baseline')
    parser = argparse.ArgumentParser()
    parser.add_argument('--traffic_reading_filename', default="D:\Desktop\qjh\GTS-change\data\PJM.csv", type=str,
